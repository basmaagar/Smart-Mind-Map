...
import type { MindMapHandle } from './components/MindMapTypes';
...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  risk: [
    ...
    'family history', 'lifestyle', 'sedentary', 'diet', 'exposure',
    'environmental', 'occupational', 'socioeconomic', 'epidemiology'
  ],
  diagnosis: [
...
// BFS to build ordered node hierarchy from elements
...
const App: React.FC = () => {
  ...
  // ADDED: ref to access cy instance for PNG export
  ...
  // PNG uses cy.png() directly — captures canvas exactly as displayed
  ...
  const handleExportPDF = () => {
    exportPDF(finalElements, projectTitle);
    setShowExportMenu(false);
  };

  const handleExportMarkdown = () => {
    exportMarkdown(finalElements, projectTitle);
    setShowExportMenu(false);
  };

  const handleExportJSON = () => {
    exportJSON(finalElements, projectTitle);
    setShowExportMenu(false);
  };
  ...
  const allElements = [
    ...
    ...pendingSuggestions.map(sug => ({
      group: 'nodes',
      classes: 'suggestion',
      data: { id: `sug-${sug.name.toLowerCase().trim()}`, label: sug.name, isSuggestion: true, evidence: sug.evidence, parentId: sug.parent, suggestionObj: sug }
    })),
    ...pendingSuggestions.map(sug => ({
      group: 'edges',
